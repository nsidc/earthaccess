...
[`search_datasets`](/api/api.md/#earthaccess.api.search_datasets) allows you to search for NASA datasets using combinations of keywords.  As a simple example, we use the `keyword` keyword to search for all datasets matching `icesat-2`.
