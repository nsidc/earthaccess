...
from icechunk import IcechunkStore, S3StaticCredentials, s3_static_credentials, s3_refreshable_credentials, s3_storage
...
class S3IcechunkCredentials:
    ...
    def __call__(self) -> S3StaticCredentials:
        ...
        creds = earthaccess.__auth__.get_s3_credentials(daac=self.daac, provider=self.provider)
...
def _open_icechunk_from_url(datacube_url:str) -> IcechunkStore:
    ...
    # How woudl this support e.g. http, which other protocols make sense?
    #TODO: for now error out on everything that is not s3:
    parsed = urlparse(datacube_url)
    protocol = parsed.scheme or "file" #not sure this is needed here? will there ever be a need to auth local stores that point to EDL buckets?
    ...
    # TODO: accomodate case without virtual chunk containers.
    ...
    approved_virtual_container_urls: Dict[str, List[str]] = {'PODAAC':['s3://podaac-ops-cumulus-protected/']}

    # Provide appropriate auth for approved virtual container urls
    container_credentials = ic.containers_credentials(
        {
            url: ic.s3_refreshable_credentials(S3IcechunkCredentials(daac=daac, provider=None))#TODO: get provider from the DAACS
