...
def get_virtual_chunk_credentials(
    ...
    if config:
        ...
        raise ValueError(f"Got empty config from {storage=} and cannot infer chunk containers.")
...
def open_icechunk_from_url(
    ...
    repo = ic.Repository.open(
        storage=storage, authorize_virtual_chunk_access=virtual_chunk_credentials
    )
