import pytest
from earthaccess.icechunk_opener import _get_credential_endpoint, get_virtual_chunk_credentials, S3IcechunkCredentials
import icechunk as ic
import pathlib
# temporary jankyness until https://github.com/nsidc/earthaccess/pull/1154 is ready
from earthaccess.icechunk_opener import credential_endpoint_mapping as endpoints
from icechunk.credentials import AnyS3Credential
...
def empty_config_icechunk(tmp_path):
    ...
    storage = ic.storage.local_filesystem_storage(str(tmp_path.joinpath("empty_config_icechunk")))
...
def no_vcontainers_icechunk(tmp_path):
    ...
    # Create storage in temp directory
    storage = ic.storage.local_filesystem_storage(str(tmp_path.joinpath("no_vcontainers_icechunk")))
...
def partial_virtual_icechunk(tmp_path):
    ...
    ic.Repository.create(storage=storage, config=config, authorize_virtual_chunk_access=credentials)
