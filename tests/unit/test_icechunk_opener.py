...
from icechunk import S3StaticCredentials, AnyCredential
...
class Test_get_virtual_chunk_credentials:
    ...
    @patch('earthaccess.__auth__.get_s3_credentials')
    def test_get_partial_virtual_chunk_credentials(self, mock_get_s3_credentials, partial_virtual_icechunk):
    ...
    @patch('earthaccess.__auth__.get_s3_credentials')
    def test_get_mixed_virtual_chunk_credentials(self, mock_get_s3_credentials, mixed_virtual_icechunk):
        
        # Make sure that only the endpoints that match get a return!
        def mock_credentials(url):
            if url in ['https://archive.podaac.earthdata.nasa.gov/s3credentials',"https://data.gesdisc.earthdata.nasa.gov/s3credentials"]:
                return {
                    "accessKeyId": "ak",
                    "secretAccessKey": "sk",
                    "expiration": "2010",
        ...
        assert len(creds) == 2
        assert set(creds.keys()) == set(["s3://podaac-ops-cumulus-protected/", "s3://gesdisc-cumulus-prod-protected/"]) 
        assert all([isinstance(c, AnyCredential) for c in creds.values()])

    @patch('earthaccess.__auth__.get_s3_credentials')
    def test_get_full_virtual_chunk_credentials(self, mock_get_s3_credentials, full_virtual_icechunk):
        # Make sure that only the endpoints that match get a return!
        def mock_credentials(url):
            if url == 'https://archive.podaac.earthdata.nasa.gov/s3credentials':
                return {
